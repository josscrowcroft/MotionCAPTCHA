...
jQuery.fn.motionCaptcha || (function($) {
	...
	$.fn.motionCaptcha = function(options) {
		...
		return this.each(function() {
			if (options == 'shape-switcharoo') {
				$canvas = $($(this).data('canvasId'));
				ctx = $canvas[0].getContext("2d");
				ctx.clearRect(0, 0, $canvas.width(), $canvas.height());
				shapes = $(this).data('shapes').split(',');
				$canvas.removeClass(function(i, c) {
					return $.inArray(c, $.fn.motionCaptcha.defaults.shapes) ? c : '';
				});
			...
			// Set up Harmony vars:
			var brush, locked = false;

			// Set up MotionCAPTCHA form and jQuery elements:
			var $body = $('body'), $form = $(this), $container = $(opts.divId), $canvas = $(opts.canvasId);
			$form.data('canvasId', opts.canvasId);
			// Set up MotionCAPTCHA canvas vars:
			var canvasWidth = $canvas.width(), canvasHeight = $canvas.height(), borderLeftWidth = 1 * $canvas.css('borderLeftWidth').replace('px', ''), borderTopWidth = 1 * $canvas.css('borderTopWidth').replace('px', '');
			...
			var touchStartEvent = function(event) {
				...
				var pos = getPos(event), x = pos[0], y = pos[1];
				...
				if (!locked && _isDown) {
					...
					if (_points.length >= 10) {
						...
						if ($canvas.attr('class').match(result.Name) && result.Score > 0.7) {
							...
							ctx.fillText(opts.successMsg, canvasWidth - 10, 24);
							...
							ctx.fillText(opts.errorMsg, canvasWidth - 10, 24);
						...
						// Write error message into canvas:
						ctx.fillText(opts.errorMsg, 210, 24);
	...
	$.fn.motionCaptcha.defaults = {
		...
		onSuccess : function($form, $canvas, ctx) {
			var opts = this, $submit = opts.submitId ? $form.find(opts.submitId) : $form.find('input[type=submit]:disabled');
	...
	Ribbon.prototype = {
		...
		interval : null,
		init : function(ctx) {
			var scope = this, userAgent = navigator.userAgent.toLowerCase(), brushSize = (userAgent.search("android") > -1 || userAgent.search("iphone") > -1 ) ? 2 : 1, strokeColor = [0, 0, 0];
	...
	**/
	...
	var NumTemplates = 16, NumPoints = 64, SquareSize = 250.0, Origin = NewPoint(0, 0);
	...
	function DollarRecognizer() {
		...
		this.Recognize = function(points) {
			var b = +Infinity, t = 0, radians, i, score, vector;
	...
	function Resample(points, n) {
		...
		D = 0.0, newpoints = new Array(points[0]), i;
		for ( i = 1; i < points.length; i++) {
			var d = Distance(points[i - 1], points[i]);
			if ((D + d) >= I) {
				var qx = points[i - 1].X + ((I - D) / d) * (points[i].X - points[i - 1].X), qy = points[i - 1].Y + ((I - D) / d) * (points[i].Y - points[i - 1].Y), q = NewPoint(qx, qy);
	...
	function RotateBy(points, radians) {
		var c = Centroid(points), cos = Math.cos(radians), sin = Math.sin(radians), newpoints = [], i;
		for ( i = 0; i < points.length; i++) {
			var qx = (points[i].X - c.X) * cos - (points[i].Y - c.Y) * sin + c.X, qy = (points[i].X - c.X) * sin + (points[i].Y - c.Y) * cos + c.Y;
	...
	function ScaleTo(points, size) {
		var B = BoundingBox(points), newpoints = [], i;
		for ( i = 0; i < points.length; i++) {
			var qx = points[i].X * (size / B.Width), qy = points[i].Y * (size / B.Height);
			newpoints[newpoints.length] = NewPoint(qx, qy);
		}
		return newpoints;
	}
	...
	function TranslateTo(points, pt) {
		var c = Centroid(points), newpoints = [], i;
		for ( i = 0; i < points.length; i++) {
			var qx = points[i].X + pt.X - c.X, qy = points[i].Y + pt.Y - c.Y;
			newpoints[newpoints.length] = NewPoint(qx, qy);
		}
		return newpoints;
	}
	...
	function Vectorize(points) {// for Protractor
		var sum = 0.0, vector = [], i, magnitude;
		for ( i = 0; i < points.length; i++) {
